...
**Interfaces:**
- Consumes: `harness.config.load_sources`, `harness.config.load_models`
- Produces:
  - `scripts.fetch_data.verify(path, expected_sha256, expected_bytes) -> None` — raises `ChecksumError`
  - `scripts.fetch_data.ChecksumError`
  - `scripts.fetch_data.main(argv) -> int` — CLI: `--root`, `--out`, writes `<out>/<dataset>/...` plus `<out>/masks/`
...
def main(argv=None) -> int:
    ...
    for model in load_models(root).values():
        shutil.copy2(root / model.mask, masks_out / f"{model.id}.nii.gz")
...
%   serializer. One writer for all MATLAB targets keeps it a property of the fit.
    ...
    plain = fullfile(outDir, [base '.nii']);
