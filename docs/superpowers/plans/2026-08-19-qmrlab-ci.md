...
**Architecture:** A GitHub Actions workflow with five jobs — `prepare-data` fetches and verifies the canonical inputs once, `plan` generates lane-split matrices from `targets/*/target.yml`, `fit-matlab` and `fit-rust` run each target, and `analyze` + `publish` turn the collected outputs into a static site. Adapters report only facts (status, timing, units, paths); every derived number is computed once in the Python `harness` package.
...
- **No Docker.** A target's `lane` names its TOOLCHAIN: `matlab`, `rust`, and reserved
  `python` / `julia` / `r` for future softwares. (Spec §3.4)
...
- Produces:
  ...
  - `harness.matrix.build_matrices(targets: dict[str, TargetSpec]) -> dict[str, list[dict]]` — one key per lane in `harness.config.KNOWN_LANES`, empty list where a lane has no targets
  - `harness.matrix.main(argv) -> int` — CLI writing one `<lane>=<json>` line per lane to `$GITHUB_OUTPUT`
...
lane needs setup-matlab and run-command, a rust lane needs a Rust toolchain instead, and a job
...
> **Amended during execution (ruling R6).** As first written, `from_dict` let structurally
> invalid `maps` (a dict, or string entries) escape as `AttributeError`, and enforced no status
> invariants — `failed` with maps parsed, `ok` with zero maps parsed. Shipped code type-checks
> `maps` and each entry, and requires `ok` to carry at least one map while `failed` and
> `not_applicable` carry none. This classmethod is the gate `scripts/emit_record.py` relies on,
> so a bare `AttributeError` here is an unexplained shell-adapter crash.
...
> **Amended during execution (ruling R5).** As first written this function wrapped nothing in
> `try`/`except`, so an undeclared map name, a missing map file, a voxel-count mismatch or an
> unknown model each aborted the entire analysis and discarded every other target's results —
> contradicting spec §10, which permits exactly two run-aborting failures. Shipped code isolates
> per record, emits an `analysis failed: ...` record for the offender, re-raises only
> `ConfigError`, and stages canonical values so a target failing on its Nth map contributes none.
...
> **Amended during execution.** This step originally carried a verbatim copy of the
> era-3 driver. That copy is now STALE: `qmrlab_ci_era3.m` gained five fixes during
> Task 14 (array-safe timing encoding, a `qmrlab_ci_field_candidates` resolver with
> `resolved{}` tracking, a voxel-count fallback that uses the resolved field, a `.mat`
> double cast, and a harness-owned NIfTI reader). Transcribing the old snapshot would
> reintroduce all five. Derive from the shipped file instead — it cannot go stale.

Copy `matlab/qmrlab_ci_era3.m` to `matlab/qmrlab_ci_era2.m` and change exactly three things:

1. the function name on line 1, to `qmrlab_ci_era2`;
2. the H1 comment, to describe the CamelCase object API (qMRLab v2.0.0 – v2.0.5);
3. the loader call, from `qmrlab_ci_load_era3` to `qmrlab_ci_load_era2`.

Change nothing else. In particular keep, unmodified: the `resolved{}` field-resolution
loop, `record.timing.fit_seconds = num2cell(times);`, the `nVox` fallback that reads
`FitResults.(resolved{1})`, and the per-model try/catch that records a failure and
continues to the next model.

Verify with `diff matlab/qmrlab_ci_era3.m matlab/qmrlab_ci_era2.m` that ONLY those three
lines differ. If more differ, you have hand-edited something you should not have.
...
> **Amended during execution.** The code below predates Task 14's five fixes. Unlike the
> era-2 driver, this one cannot simply be copied from era 3 — V1's `FitData` takes
> `(data, Prot, FitOpt, Method, wait)` rather than a model object, and V1 supports only
> `qmt_spgr`. So transcribe the structure below, but adopt these four patterns from the
> SHIPPED `matlab/qmrlab_ci_era3.m`, reading that file rather than any snapshot:
>
> 1. **Timing must encode as an array.** Use
>    `record.timing = struct('repeats', n, 'n_voxels_fitted', nVox);` followed by
>    `record.timing.fit_seconds = num2cell(times);`. `jsonencode` writes a 1-element
>    numeric array as a bare scalar, and V1's repeat count IS 1, so the naive form
>    produces a record the schema rejects — every single time, for this target.
> 2. **Resolve output fields through `qmrlab_ci_field_candidates`**, tracking `resolved{}`
>    exactly as era 3 does. V1 returns `F, kf, kr, R1f, R1r, T2f, T2r, resnorm`, which
>    should match the canonical names directly — but resolve rather than assume, so a
>    surprise fails loudly instead of silently writing the wrong map.
> 3. **Size the voxel-count fallback from `FitResults.(resolved{1})`**, never from the
>    canonical name.
> 4. **Do not call `load_nii_data`.** `qmrlab_ci_load_any` already routes NIfTI through
>    the harness-owned reader; V1's qMT inputs are all `.mat`, so this mostly will not
>    arise, but do not reintroduce a direct call.
...
### Task 17: qmrust adapter (rust lane)
...
fit_args() {
  case "$1" in
    ...
    qmt_spgr)           echo "--mat-dir $(dirname "$(find_one qmt MTdata.mat)") --config recipes/non-bids/qmt_config_sledpikerp.yaml" ;;
...
git commit -m "feat: qmrust rust-lane adapter and shell record emitter"
```
...
jobs:
  ...
  fit-rust:
    ...
    strategy:
      fail-fast: false
      matrix:
        target: ${{ fromJSON(needs.plan.outputs.rust) }}
    env:
      QMRLAB_CI_RUN_STARTED: ${{ needs.plan.outputs.run_started_utc }}
  ...
  analyze:
    needs: [plan, fit-matlab, fit-rust]
...
jobs = doc['jobs']
assert set(jobs) == {'prepare-data','plan','fit-matlab','fit-rust','analyze','publish'}
assert jobs['fit-matlab']['strategy']['fail-fast'] is False
assert jobs['fit-rust']['strategy']['fail-fast'] is False
print('workflow OK:', list(jobs))"
```

Expected: `workflow OK: ['prepare-data', 'plan', 'fit-matlab', 'fit-rust', 'analyze', 'publish']`
...
> **Carried in from Task 14's review.** The MATLAB drivers take `models` as a cellstr and
> do not validate it. A bare char id (`'vfa_t1'` rather than `{'vfa_t1'}`) makes `models{ii}`
> fail in a way that reads as a driver bug rather than a caller bug. The workflow builds that
> cellstr from a GitHub Actions expression, so this is the caller. When writing the smoke
> workflow, confirm the single-model case produces `{'b1_dam'}` and not `'b1_dam'` — the
> single-element case is exactly the one an expression-built list gets wrong, and it is the
> case smoke.yml uses.

> **Scope widened during execution (ruling R25).** This task originally smoke-tested one
> target and one model. It now covers **one target per era** — `qmrlab@V1` (era 1, R2021a),
> `qmrlab@v2.0.5` (era 2, R2021a) and `qmrlab@v3.0.0` (era 3, R2026a). Everything verified
> during implementation ran on R2026a, because that is all the development machine has, so
> **nothing has ever tested the R2021a pin** — neither that `setup-matlab` can install it, nor
> that 2016/2017-era qMRLab loads on it. That is the one failure class local verification
> cannot reach, and it governs 11 of the 14 MATLAB targets. Also: this task must DELETE
> `.github/workflows/matlab-smoke-preview.yml`, a temporary bridge added out-of-band, whose
> trigger only fires when its own file changes — so driver edits never re-run it.
...
suite plus one MATLAB target and one Rust target against a single closed-form model,
...
`README.md` already exists — it was written mid-execution at the project owner's request,
because the repo was public with nothing explaining it. Read it, and only ADD what this
task's brief covers that it is missing. Do not rewrite or replace it, and do not delete the
"under construction" note without checking that the workflows really do publish.
